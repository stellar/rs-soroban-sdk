...
use stellar_xdr::{
    ReadXdr, SpecEntry, SpecEntryFunction, SpecEntryFunctionV0, SpecTypeDef, SpecTypeUdt,
};
...
fn test_functional() {
    let e = Env::default();
    let a = Udt { a: 5, b: 7 }.into_val(&e);
    let b = Udt { a: 10, b: 14 }.into_val(&e);
    let c = __add(e.clone(), a, b);
    let c = Udt::try_from_val(&e, c).unwrap();
    assert_eq!(c, Udt { a: 15, b: 21 });
}
...
fn test_spec() {
    ...
    let expect = SpecEntry::Function(SpecEntryFunction::V0(SpecEntryFunctionV0 {
        ...
        .try_into()
        .unwrap(),
        output_types: vec![SpecTypeDef::Udt(SpecTypeUdt {
            name: "Udt".try_into().unwrap(),
        })]
        .try_into()
        .unwrap(),
