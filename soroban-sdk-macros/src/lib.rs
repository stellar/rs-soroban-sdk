    ...
    if cfg!(feature = "testutils") {
        output.extend(quote! {
            mod #fn_set_registry_ident {
                ...
                static FUNCS: Mutex<BTreeMap<&'static str, &'static #crate_path::testutils::F>> = Mutex::new(BTreeMap::new());

                pub(crate) fn register(name: &'static str, func: &'static #crate_path::testutils::F) {
                ...
                pub(crate) fn call(name: &str, env: #crate_path::Env, args: &[#crate_path::Val]) -> Option<#crate_path::Val> {
                    let fopt: Option<&'static #crate_path::testutils::F> = FUNCS.lock().unwrap().get(name).map(|f| f.clone());
                    fopt.map(|f| f(env, args))
                }
            }

            impl #crate_path::testutils::ContractFunctionRegister for #ty {
                fn register(name: &'static str, func: &'static #crate_path::testutils::F) {
