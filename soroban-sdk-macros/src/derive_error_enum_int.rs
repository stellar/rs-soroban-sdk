...
pub fn derive_type_error_enum_int(
    ...
    // contract crate encodes to XDR at compile time. An error enum holds no
    // types, so it contains no reference to carry an id and this is already the
    ...
    let entry_ref = || {
        ...
        let cases = spec_enum.cases.iter().map(|c| {
            ...
            let value = c.value;
            quote!(#path::xdr::ScSpecUdtErrorEnumCaseV0Ref { doc: #doc, name: #name, value: #value })
        });
        quote! {
            #path::xdr::ScSpecEntryRef::UdtErrorEnumV0(#path::xdr::ScSpecUdtErrorEnumV0Ref {
                doc: #doc,
                lib: #lib,
                name: #name,
