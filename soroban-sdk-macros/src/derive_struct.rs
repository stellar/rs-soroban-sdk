...
pub fn derive_type_struct(
    ...
    // identity even when its own spec is not exported. UDT references carry
    // placeholder (zeroed) ids; each is resolved to the referenced type's id at
    // const-eval time by the generated `spec_xdr`.
    ...
    let spec_gen = crate::map_type::referenceable_spec_gen(&spec_entry, &ref_types, ident, spec);
