...
# stream of pretty formatted XDR-JSON values, sorted so that moving items around
# in the source does not reorder them. Serves to surface changes to the spec the
# SDK embeds, which the expanded code does not show because the spec is encoded
