...
impl Events {
    ...
    pub fn transfer_with_muxed_address(&self, from: MuxedAddress, to: MuxedAddress, amount: i128) {
        let from_muxed_id = from.id();
        let to_muxed_id = to.id();
        let topics = (symbol_short!("transfer"), from, to);
        let amount_val = amount.into_val(&self.env);
        let data = match (from_muxed_id, to_muxed_id) {
            (None, None) => amount_val,
            (None, Some(to_muxed_id)) => self
                .env
                .map_new_from_slices(
                    &["amount", "to_muxed_id"],
                ...
                .unwrap()
                .into(),
            (Some(from_muxed_id), None) => self
                .env
                .map_new_from_slices(
                    &["amount", "from_muxed_id"],
                    &[amount_val, from_muxed_id.into_val(&self.env)],
                )
                .unwrap()
                .into(),
            (Some(from_muxed_id), Some(to_muxed_id)) => self
                .env
                .map_new_from_slices(
                    &["amount", "from_muxed_id", "to_muxed_id"],
                    &[
                        amount_val,
                        from_muxed_id.into_val(&self.env),
                        to_muxed_id.into_val(&self.env),
                    ],
                )
                .unwrap()
                .into(),
