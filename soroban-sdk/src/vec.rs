...
use super::{
    ...
    xdr::ScObjectType,
    ConversionError, Env, Object, RawVal, Set, TryFromVal, TryIntoVal,
};
...
macro_rules! impl_into_vec_for_tuple {
    ( $($typ:ident $idx:tt)* ) => {
            ...
            $($typ: TryIntoVal<Env, RawVal>),*
        {
            ...
            fn try_from_val(env: &Env, val: &($($typ,)*)) -> Result<Vec<RawVal>, Self::Error> {
                Ok(vec![&env, $(val.$idx.try_into_val(env)?.map_err(|_| ConversionError), )*])
            }
        }
    };
}
...
impl<T> Eq for Vec<T> where T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal> {}

impl<T> PartialEq for Vec<T>
where
    T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal>,
{
...
where
    T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal> + Debug + Clone,
    <T as TryIntoVal<Env, RawVal>>::Error: Debug,
{
    ...
    T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal>,
{
    ...
    #[inline(always)]
    pub fn get(&self, i: u32) -> Option<Result<T, <T as TryFromVal<Env, RawVal>>::Error>> {
        if i < self.len() {
            let env = self.env();
    ...
    #[inline(always)]
    pub fn get_unchecked(&self, i: u32) -> Result<T, <T as TryFromVal<Env, RawVal>>::Error>
    where
        <T as TryIntoVal<Env, RawVal>>::Error: Debug,
    {
        let env = self.env();
    ...
    pub fn set(&mut self, i: u32, v: T) {
        let env = self.env();
        self.obj = env.vec_put(self.obj, i.into(), v.try_into_val(env).unwrap_optimized());
    }
    ...
    pub fn push_front(&mut self, x: T) {
        let env = self.env();
        self.obj = env.vec_push_front(self.obj, x.try_into_val(env).unwrap_optimized());
    }

    #[inline(always)]
    pub fn pop_front(&mut self) -> Option<Result<T, <T as TryIntoVal<Env, RawVal>>::Error>> {
        let last = self.last()?;
        let env = self.env();
    ...
    #[inline(always)]
    pub fn pop_front_unchecked(&mut self) -> Result<T, <T as TryIntoVal<Env, RawVal>>::Error> {
    ...
    pub fn push_back(&mut self, x: T) {
        let env = self.env();
        self.obj = env.vec_push_back(self.obj, x.try_into_val(env).unwrap_optimized());
    }

    #[inline(always)]
    pub fn pop_back(&mut self) -> Option<Result<T, <T as TryIntoVal<Env, RawVal>>::Error>> {
        let last = self.last()?;
        let env = self.env();
    ...
    #[inline(always)]
    pub fn pop_back_unchecked(&mut self) -> Result<T, <T as TryIntoVal<Env, RawVal>>::Error> {
    ...
    #[inline(always)]
    pub fn pop(&mut self) -> Option<Result<T, <T as TryIntoVal<Env, RawVal>>::Error>> {
        self.pop_back()
    }

    #[deprecated(note = "use [Vec::push_back_unchecked]")]
    #[inline(always)]
    pub fn pop_unchecked(&mut self) -> Result<T, <T as TryIntoVal<Env, RawVal>>::Error> {
        self.pop_back_unchecked()
    }

    #[inline(always)]
    pub fn first(&self) -> Option<Result<T, <T as TryIntoVal<Env, RawVal>>::Error>> {
        if self.is_empty() {
            None
        } else {
            let env = &self.env;
    ...
    #[inline(always)]
    pub fn first_unchecked(&self) -> Result<T, <T as TryIntoVal<Env, RawVal>>::Error> {
        let env = &self.env;
        let val = env.vec_front(self.obj);
        T::try_from_val(env, &val)
    }
    ...
    #[inline(always)]
    pub fn last(&self) -> Option<Result<T, <T as TryIntoVal<Env, RawVal>>::Error>> {
        if self.is_empty() {
            None
        } else {
            let env = self.env();
    ...
    #[inline(always)]
    pub fn last_unchecked(&self) -> Result<T, <T as TryIntoVal<Env, RawVal>>::Error> {
        let env = self.env();
        let val = env.vec_back(self.obj);
        T::try_from_val(env, &val)
    }
    ...
    pub fn insert(&mut self, i: u32, x: T) {
        let env = self.env();
        self.obj = env.vec_put(self.obj, i.into(), x.try_into_val(env).unwrap_optimized());
    }
    ...
    where
        T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal> + Clone,
    {
    ...
    pub fn iter_unchecked(
        &self,
    ) -> UncheckedIter<VecIter<T>, T, <T as TryIntoVal<Env, RawVal>>::Error>
    where
        T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal> + Clone,
        <T as TryIntoVal<Env, RawVal>>::Error: Debug,
    {
    ...
    T: TryIntoVal<Env, RawVal>,
{
    ...
    pub fn contains(&self, item: impl Borrow<T>) -> bool {
        let env = self.env();
        let val = item.borrow().try_into_val(env).unwrap_optimized();
...
where
    T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal>,
    Vec<T>: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal>,
    T: Clone,
    <T as TryIntoVal<Env, RawVal>>::Error: Debug,
{
    #[inline(always)]
...
where
    T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal>,
{
    type Item = Result<T, <T as TryIntoVal<Env, RawVal>>::Error>;
...
impl<T> FusedIterator for VecIter<T> where T: TryIntoVal<Env, RawVal> + TryFromVal<Env, RawVal> {}
