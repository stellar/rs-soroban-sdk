...
// frame's function name. A `fn` can't capture, so the address it authorizes is
// passed in through a thread-local.
std::thread_local! {
    static ADMIN: std::cell::RefCell<Option<Address>> = const { std::cell::RefCell::new(None) };
}
