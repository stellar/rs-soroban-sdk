...
//!    [`Deployer::update_current_contract`]. Both take a [`ContractExecutable`], which is either a
//!    Wasm hash or a reference to an executable reference entry owned by a contract (see
//!    [`ExecutableRefs`]). A [`BytesN<32>`] Wasm hash, or a `[u8; 32]`, converts into one and is
//!    still accepted directly, so code passing a Wasm hash to [`deploy_v2`] does not change. Code
//!    passing a [`Val`] or [`ScVal`] does change: it still compiles, but the value is now read as an
//!    encoded [`ContractExecutable`] rather than as a Wasm hash, so convert it to a [`BytesN<32>`]
//!    first.
//!
//!    ```
...
//!
//!    The two types are ABI-compatible. [`Hash<32>`] contains a [`BytesN<32>`].
//!    For [`CustomAccountInterface::__check_auth`], the host constructs the
//!    `signature_payload`, so it is guaranteed to contain bytes produced by the
//!    host's authentication payload hash. This guarantee is specific to
//!    host-managed `__check_auth` invocations and does not apply to general
