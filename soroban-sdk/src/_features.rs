...
//! ## `fuzzcheck`
//!
//! Enables the `testutils::fuzzcheck` module, providing [`fuzzcheck`](https://github.com/loiclec/fuzzcheck-rs) mutators for
//! contract types so that contracts can be fuzzed with fuzzcheck. Implies
//! `testutils`.
//!
//! This feature requires a nightly compiler, because fuzzcheck itself requires
//! one. Crates that enable it and that contain `#[contracttype]` types must
//! declare `#![feature(coverage_attribute)]` and, if they are `no_std`,
//! `extern crate std`, both of which the code generated by fuzzcheck's derive
//! macros requires.
//!
//! ## `alloc`
//!
