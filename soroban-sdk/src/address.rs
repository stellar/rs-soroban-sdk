...
impl Address {
    ...
    /// has authorized the invocation. This is similar in effect to calling
    /// `require_auth` on the `address`, but unlike `require_auth` this is not
    /// interpreted as a new contract invocation and thus does not require a
    ...
    ///
    ///     // Attach `delegate` to the transaction as a delegated signer of the
    ///     // account. This is normally done by the user's wallet/tooling; the
    ///     // raw authorization entry is built and set here (hidden for brevity).
    /// #     use soroban_sdk::xdr::{
    ...
    /// #     // Delegates must be sorted by address with no duplicates.
