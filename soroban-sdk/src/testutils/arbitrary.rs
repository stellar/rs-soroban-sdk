...
//! implementing the [`Arbitrary`] trait that `cargo-fuzz` relies on to feed
//! fuzzers with generated Rust values.
//!
//! The same generated values can drive `proptest` property tests, which run as
//! ordinary tests under `cargo test` with no fuzzing toolchain, by bridging
//! `Arbitrary` to a proptest [`Strategy`]. See [Property testing with
//! `proptest`](#property-testing-with-proptest).
//!
...
//!
//! Prototypes are not only for `cargo-fuzz`: because every
//! [`SorobanArbitrary::Prototype`] implements [`Arbitrary`], the same values
//! can be generated by [`proptest`], which runs property tests as ordinary
//! `#[test]`s under `cargo test`, with no fuzzing toolchain.
//!
//! `proptest` generates values from [`Strategy`]s, not from `Arbitrary`
//! implementations, so the two are bridged by the
//! [`proptest-arbitrary-interop`] crate: its `arb` function turns any
...
//! Cargo feature is active, so the contract crate must enable its own
//! "testutils" feature — the one that turns on "soroban-sdk/testutils" — when
//! the property tests are compiled.
//!
...
//!         // call the contract with `address` and `deposit_amount`, and
//!         // assert its properties with `prop_assert!`
//!     }
//! }
//! # fn main() { }
//! ```
...
//!         // call the contract with `claim`, and assert its properties with
//!         // `prop_assert!`
//!     }
//! }
//! # fn main() { }
