...
//! values, and which both `cargo-fuzz` and `proptest` can be driven from.
//!
//! The same generated values can drive `proptest` property tests by bridging
...
//! Shrinking is limited across this bridge: `arb`'s simplify step truncates the
//! generated byte buffer and regenerates from it, so the counterexample proptest
//! reports is a fresh sample rather than a minimal reduction of the value that
//! failed.
//!
...
//! addresses, never to account (`G...`) addresses, so a property test that also
//! needs account addresses must construct them itself.
//!
//!
