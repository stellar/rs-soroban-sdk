//! Support for generating random Soroban contract types, for fuzzing with
//! [`cargo-fuzz`] and for property testing with [`proptest`].
//!
...
//! - provides the `arb` and `arb_sized` `proptest` strategies, and reexports
//!   the `proptest` crate, when the "testutils-proptest" Cargo feature is
//!   defined.
//!
...
//!
//! The same prototypes can be generated by [`proptest`], with the `arb`
//! strategy. It is available when the "testutils-proptest" Cargo feature is
//! defined, which implies the "testutils" feature.
//!
//! ```toml
//! [dev-dependencies]
//! proptest = "1"
...
//! use soroban_sdk::testutils::arbitrary::arb;
...
//!
//! An [`Address`][crate::Address] prototype generates account addresses
//! (strkeys beginning with `G`) about as often as contract addresses (strkeys
//! beginning with `C`). Account addresses are not registerable, so
//! `env.register_at(&addr, ...)` panics with "not a contract address" for
//! about half of generated addresses. Tests and fuzz targets that need a
//! contract address must generate one another way, or filter. Authorization
//! works for both kinds: `require_auth` succeeds for an account address under
//! `mock_all_auths`.
...
mod objects {
    ...
    /// The prototype of [`Address`], generating both account addresses
    /// (strkeys beginning with `G`) and contract addresses (strkeys beginning
    /// with `C`), at roughly equal rates.
    ///
    /// Note that account addresses are not registerable: calling
    /// `env.register_at(&addr, ...)` with an account address panics with "not
    /// a contract address". Fuzz targets and property tests that register the
    /// generated address as a contract must generate a contract address
    /// another way, or filter the generated addresses. Authorization works for
    /// both kinds of address: `require_auth` succeeds for an account address
    /// under `mock_all_auths`.
    #[derive(Arbitrary, Debug, Clone, Eq, PartialEq, Ord, PartialOrd)]
    pub enum ArbitraryAddress {
        Account { inner: [u8; 32] },
        Contract { inner: [u8; 32] },
    }
    ...
    impl TryFromVal<Env, ArbitraryAddress> for Address {
        ...
        fn try_from_val(env: &Env, v: &ArbitraryAddress) -> Result<Self, Self::Error> {
            use crate::env::xdr::{AccountId, ContractId, Hash, PublicKey, ScAddress, Uint256};

            let sc_addr = match v {
                ArbitraryAddress::Account { inner } => ScVal::Address(ScAddress::Account(
                    AccountId(PublicKey::PublicKeyTypeEd25519(Uint256(*inner))),
                )),
                ArbitraryAddress::Contract { inner } => {
                    ScVal::Address(ScAddress::Contract(ContractId(Hash(*inner))))
                }
            };
    ...
    impl TryFromVal<Env, ArbitraryBls12381Fp> for Bls12381Fp {
        ...
        fn try_from_val(env: &Env, v: &ArbitraryBls12381Fp) -> Result<Self, Self::Error> {
            ...
            bytes[0] %= 0x1a;
            Ok(Bls12381Fp::from_array(env, &bytes))
        }
    }
    ...
    impl TryFromVal<Env, ArbitraryBls12381Fp2> for Bls12381Fp2 {
        ...
        fn try_from_val(env: &Env, v: &ArbitraryBls12381Fp2) -> Result<Self, Self::Error> {
            ...
            Ok(Bls12381Fp2::from_array(env, &bytes))
        }
    }
    ...
    impl SorobanArbitrary for Bn254Fp {
        type Prototype = ArbitraryBn254Fp;
    }

    impl TryFromVal<Env, ArbitraryBn254Fp> for Bn254Fp {
        type Error = ConversionError;
...
/// Support for generating Soroban prototypes with the `proptest` crate.
///
/// This module is only available when the "testutils-proptest" Cargo feature
/// is defined.
#[cfg(feature = "testutils-proptest")]
mod proptest_strategies {
    use super::api::SorobanArbitrary;
    use core::fmt::Debug;
    ...
    /// are small — see the [module docs][crate::testutils::arbitrary] for the
    /// distribution and how to generate larger collections.
    ///
    /// [`proptest`]: ::proptest
    /// [`Arbitrary`]: ::arbitrary::Arbitrary
    /// [`Env`]: crate::Env
