...
//!   the `proptest` crate, when the "proptest" Cargo feature is defined.
//!
...
//! strategy. It is available when the "proptest" Cargo feature is defined,
//! which implies the "testutils" feature.
//!
//! ```toml
//! [dev-dependencies]
//! proptest = "1"
//! soroban-sdk = { version = "*", features = ["proptest"] }
//! ```
//!
...
//!
//! An [`Address`][crate::Address] prototype only generates contract addresses
//! (strkeys beginning with `C`), never account addresses (strkeys beginning
//! with `G`). Tests that need an account address must construct one another
//! way, such as with [`Address::from_str`][crate::Address::from_str].
...
///
/// This module is only available when the "proptest" Cargo feature is defined.
#[cfg(feature = "proptest")]
#[cfg_attr(feature = "docs", doc(cfg(feature = "proptest")))]
pub use proptest;
