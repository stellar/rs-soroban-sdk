...
//! - provides the `arb` `proptest` strategy, and reexports the `proptest`
//!   crate, when the "testutils-proptest" Cargo feature is defined.
//!
...
mod proptest_strategies {
    ...
    /// The number of random bytes generated for each value, and fed to the
    /// `Arbitrary` implementation of the prototype.
        ...
        T::Prototype: Debug,
    {
        ...
        // implementation of the prototype. Shrinking is performed by proptest
        // on those bytes, which the `Arbitrary` implementation translates into
        // simpler values.
        proptest::collection::vec(proptest::num::u8::ANY, BYTES_PER_VALUE).prop_filter_map(
            "arbitrary could not generate a value from the bytes",
            |bytes| T::Prototype::arbitrary_take_rest(Unstructured::new(&bytes)).ok(),
        )
    }
}
