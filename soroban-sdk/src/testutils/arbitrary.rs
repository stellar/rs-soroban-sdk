...
//! Soroban contract.
//!
//! More sophisticated fuzzers accept not bytes but Rust types, which the
//! [`Arbitrary`] trait produces from those bytes, and which a fuzz target takes
//! as its input:
//!
//! ```
...
//! }
//!
//! fn fuzz(input: FuzzDeposit) {
//!     // fuzz the contract based on the input
//! }
//! ```
//!
//! The sections below on `cargo-fuzz` and `cargo-afl` show how each fuzzer is
//! pointed at such a function; everything in between is common to both.
//!
//!
...
//!
//! fn fuzz(input: <Vec<Address> as SorobanArbitrary>::Prototype) {
//!     let env = Env::default();
