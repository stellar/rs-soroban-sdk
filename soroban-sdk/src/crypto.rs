...
/// A `BytesN<N>` intended to contain the output of a cryptographic hash
/// function.
///
/// When a `Hash<N>` is returned by [`sha256`][Crypto::sha256] or
/// [`keccak256`][Crypto::keccak256], the SDK guarantees that the bytes came
/// from the corresponding secure cryptographic hash function. The host also
/// provides this guarantee for the first parameter of
