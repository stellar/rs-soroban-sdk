...
use crate::{
    ...
    Bytes, BytesN, ConversionError, Env, IntoVal, TryFromVal, Val, Vec,
};
...
/// [`sha256`][Crypto::sha256], [`sha512`][Crypto::sha512], [`keccak256`][Crypto::keccak256], 
...
impl Crypto {
    ...
    pub fn sha512(&self, data: &Bytes) -> Hash<64> {
        let env = self.env();
        let bin = internal::Env::compute_hash_sha512(env, data.into()).unwrap_infallible();
        unsafe { Hash(BytesN::unchecked_new(env.clone(), bin)) }
    }
    ...
    /// Verifies a bulletproof range proof for multiple committed values.
    ///
    /// The proof verifies that each committed value is in the range [0, 2^nbits).
    /// 
    /// # Arguments
    /// 
    /// * `proof_bytes` - The bulletproof range proof as bytes
    /// * `dst` - Domain separation tag used in proof generation
    /// * `nbits` - Number of bits for the range (must be 8, 16, 32, or 64)
    /// * `commitments` - Vector of value commitments (Pedersen commitments in Ristretto255 group)
    pub fn bulletproof_verify_multiple_values_in_range(
        &self,
        proof_bytes: &Bytes,
        dst: &Bytes,
        nbits: u32,
        ...
        let env = self.env();
        let _ = internal::Env::bulletproof_verify_multiple_values_in_range(
            env,
            proof_bytes.into(),
            dst.into(),
            nbits.into(),
            commitments.to_object(),
        )
        .unwrap_infallible();
    }
}
